...
int main() {
    ...
    - sizeof(SharedData): tamanho da memória */
    HANDLE hMapFile = CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, sizeof(SharedData), NOME_MEMORIA);
    // Verifica se o handle é valido, se a memória foi criada com sucesso
    if (!hMapFile) {
        logger(L"error", L"create_memory", L"Erro ao criar memória compartilhada", GetLastError(), L"system");
        return 1;
    }

    /*Mapeia a memória compartilhada para o espaço de endereço do processo
    cria um ponteiro do tipo SharedData que aponta para a memoria compartilhada (sharedMem)
...
*/
    ...
    if (!hMutex) {
        logger(L"error", L"create_mutex", L"Erro ao criar Mutex", GetLastError(), L"system");
    ...
    while (true) {
        ...
        if (input == L"sair") {
            sharedMem->encerrar_flag = true; // defini a flag de encerramento como verdadeira para poder encerrar o reader
            ...
            logger(L"info", L"exit", L"Writer solicitou encerramento", 0, L"shared_memory");
        ...
        if (!input.empty()) {
            ...
            wcsncpy(sharedMem->mensagem, input.c_str(), TAM_MEMORIA - 1); //copia a entrada para o buffer message do ponteiro sharedMem, até TAM_MEMORIA - 1 
            sharedMem->mensagem[TAM_MEMORIA - 1] = L'\0'; //O último caractere é colocado como '\0'
            ...
            logger(L"info", L"write", input, input.size(), L"shared_memory");
            logger(L"info", L"mutex liberado", L"Writer liberou o mutex", 0, L"system");
        }
